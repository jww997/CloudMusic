...
export interface PARAMS_TOP_PLAYLIST extends OFFSET {
    order: "new" | "hot"; // 最新和最热
    cat: string; // 可从歌单分类接口获取(/playlist/catlist)
}
...
export interface PARAMS_PLAYLIST_CREATE {
    ...
    type?: "NORMAL" | "VIDEO" | "SHARED" // 歌单类型, 'VIDEO'视频歌单 'SHARED'共享歌单
}

export interface PARAMS_PLAYLIST_DELETE {
    id: number | string
}
...
export interface PARAMS_ARTIST_VIDEO extends ID {
    size?: number // 返回数量
    cursor?: number // 返回数据的cursor
    order?: 0 | 1 // 0时间排序 1热度排序
}
